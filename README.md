...
O projeto não usa nenhum catálogo externo (TMDB e afins): a disponibilidade em streaming vem inteiramente do que o próprio motor respondeu, sempre marcada como não confirmada, com os links de "conferir na fonte" ao lado. O **pôster é coletado da própria resposta do motor** — o prompt já obriga cada indicação a trazer o endereço da imagem — e o servidor confirma o link antes de devolvê-lo: normaliza para `https` (em `http` o navegador bloquearia a imagem) e checa que o endereço responde mesmo com uma imagem. Quando o link do motor não responde, a coleta segue sozinha para uma imagem real e gratuita da Wikipedia (em português e depois em inglês) e, só como último recurso, para uma capa ilustrativa gerada por IA — que a tela rotula como tal, nunca como o pôster oficial. As três indicações são resolvidas em paralelo, sob o tempo que sobrou da requisição, para que a busca da imagem nunca estoure o limite da função e derrube a recomendação.
