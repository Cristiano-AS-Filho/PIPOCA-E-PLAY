...
import api_core
from api_core import (  # noqa: F401 - reexportados para compatibilidade (testes e api/recommend.py)
...
class AppHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path.path in {"/api/admin/status", "/api/admin/users"}:
            ...
            return
        if path.path == "/api/payments/plans":
            self.send_result(api_core.list_plans_route())
            return
        if path.path == "/api/payments/status":
            self.send_result(api_core.payment_status(self.current_user()))
            return
        if path.path == "/api/marks":
            self.send_result(api_core.marks_list(self.current_user()))
            return
    ...
    def do_POST(self):
        ...
        if self.path == "/api/payments/checkout":
            self.send_result(
                api_core.create_checkout(self.current_user(), self.request_json_or_empty(4_096))
            )
            return

        if self.path == "/api/payments/webhook":
            self.send_result(api_core.payment_webhook(self.headers, self.request_json_or_empty(65_536)))
            return

        if self.path == "/api/marks":
            self.send_result(
                api_core.marks_action(self.current_user(), self.request_json_or_empty(4_096))
            )
            return
