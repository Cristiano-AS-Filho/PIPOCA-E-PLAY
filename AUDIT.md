...
## Rodada 13 — a busca que falhava "às vezes" e o cartão sem onde assistir

### Diagnóstico

Dois relatos, com a mesma raiz em comum: a resposta do motor.

**1. "JSON inválido" de vez em quando.** `call_openai` pedia `max_output_tokens: 1800` com `reasoning: {"effort": "low"}` — e os tokens de raciocínio saem do mesmo teto. O JSON estrito precisa fechar três indicações completas: nove notas cada, sinopse, justificativa, prêmios, `where_to_watch` e um `poster_url` que costuma ser um link longo. Quando o modelo escrevia um pouco mais, a resposta batia no teto e vinha **cortada no meio**: `json.loads` falhava e o cliente via "O motor de recomendação retornou um JSON inválido". Daí o "por vezes" — dependia do tamanho do texto gerado. Pior: o código só olhava `status`/`incomplete_details` quando o texto vinha **vazio**; com texto truncado, ia direto para o parser e culpava o JSON.

**2. Todo cartão dizia "Onde assistir a confirmar".** O campo `where_to_watch` é obrigatório no schema, mas array vazio satisfaz o schema — e o prompt mandava justamente isso: *"Quando houver plataforma específica, trate disponibilidade como dado a ser validado por uma fonte externa, nunca como fato conhecido apenas pela IA"* e *"Não invente avaliações, plataformas, disponibilidade… Quando não tiver certeza, use 0, string vazia ou array vazio"*. Um modelo obediente devolvia `[]` sempre. O front-end então caía no texto padrão. Ou seja: o dado que é o core do produto estava sendo pedido para ficar em branco.

### Correções aplicadas

**Resposta cortada (`recommender.py`).** `MAX_OUTPUT_TOKENS = 6000` (teto não é cobrado, só o que é gerado). `_read_engine_result` passou a distinguir os casos: `status: "incomplete"` com `reason: "max_output_tokens"`, texto vazio, JSON que não fecha e payload fora do formato — cada um com a sua mensagem, nenhuma nomeando o provedor. `call_openai` virou um laço de até duas tentativas: a segunda só acontece quando ainda restam pelo menos 18s do orçamento de 52s da função (para não estourar os 60s da Vercel e devolver nada), e pede 1,5× mais espaço de saída. Na segunda falha, a mensagem diz que a resposta veio cortada — e `api_core.recommend` devolve o crédito, como já fazia.

**Onde assistir (`recommender.py`, `public/app.html`, `public/index.html`).** O prompt ganhou `_where_to_watch_prompt_line()`: o campo **não pode voltar vazio**, `platform` recebe o nome do serviço como o público brasileiro o conhece e `type` distingue `assinatura`, `aluguel_compra` e `cinema`; a justificativa dada ao modelo é honesta — a tela exibe a disponibilidade como não confirmada e oferece o link para conferir na fonte, então um serviço provável vale mais que um campo em branco. As duas frases que mandavam calar foram reescritas: "não invente" continua valendo para notas, preços, datas, classificação e prêmios, e a marcação de plataforma passou a ser descrita como restrição de escolha, não como fato a ser validado por terceiros.

No backend, `_apply_where_to_watch` limpa a resposta: apelidos viram o nome oficial (`canonical_platform`: "hbo max" → "Max (HBO)", "prime video" → "Amazon Prime Video"), serviços fora da lista oficial são mantidos como o motor escreveu (Globoplay, por exemplo), duplicatas somem, o tipo inválido vira `assinatura` e a lista é limitada a quatro. Quando o motor ainda assim cala **e o cliente marcou uma única plataforma**, o campo é completado com ela — repetir a restrição que o próprio motor usou para escolher o título não é inventar, e a tela segue dizendo que a disponibilidade não está confirmada. Com várias marcações, o campo fica vazio: não dá para saber qual delas exibe cada título.

No cartão (resultado e histórico, nas duas páginas), o canto passou a ter duas linhas: o serviço — com "(aluguel)" ou "No cinema" quando é o caso — e, abaixo, "disponibilidade a confirmar". O rodapé do cartão já levava ao JustWatch.

### Validação

`python3 -m unittest test_app` — 131 testes verdes (9 novos). Os principais são as duas regressões: uma resposta cortada com `reason: "max_output_tokens"` que **se recupera sozinha** na segunda tentativa (e pede mais espaço nela), e o cartão que recebe a plataforma marcada quando o motor devolve o campo vazio. Também cobertos: o teto de saída comportando três indicações completas, a segunda resposta cortada virando mensagem honesta ("cortada", nunca "JSON inválido", nunca o nome do provedor), a ausência de segunda tentativa quando o orçamento da função acabou, a normalização dos apelidos de plataforma, o campo que continua vazio com várias marcações, e o prompt que não carrega mais a instrução antiga.
